"""
WebSocket module for the System Health live-update feed.

Endpoint : WS /ws/system-health/live

Pushes a SystemLiveUpdate payload every 5 seconds to all connected clients.
...
* Redis Pub/Sub ready: swap the _fetch_live_snapshot() body for a Redis
  subscriber that listens on a "system-health:live" channel.
* Kafka ready: replace the DB snapshot call in _fetch_live_snapshot() with an
  AIOKafka consumer reading from a "system-health-events" topic.
...
from app.schemas.system_health import SystemLiveUpdate
...
class ConnectionManager:
    """
    Manages all active WebSocket connections for the live System Health feed.
    ...
    async def connect(self, websocket: WebSocket) -> str:
        ...
        logger.info(
            "WS client connected: %s (total=%d)", client_id, len(self._connections)
        )
        return client_id
    ...
    def disconnect(self, client_id: str) -> None:
        ...
        logger.info(
            "WS client disconnected: %s (total=%d)", client_id, len(self._connections)
        )
    ...
    async def broadcast(self, payload: dict) -> None:
        ...
        for client_id, websocket in list(self._connections.items()):
            try:
                ...
                logger.warning(
                    "WS broadcast failed for client %s: %s", client_id, exc
                )
                dead.add(client_id)
    ...
    def start_push_loop(self, client_id: str) -> None:
        ...
        The task pushes a fresh SystemLiveUpdate every _PUSH_INTERVAL
        seconds until the client disconnects or the task is cancelled.
        """
        task = asyncio.create_task(
            self._push_loop(client_id),
            name=f"system-health-push-{client_id}",
        )
    ...
    async def _push_loop(self, client_id: str) -> None:
        ...
        while client_id in self._connections:
            try:
                ...
                logger.exception(
                    "Push loop error for client %s: %s", client_id, exc
                )
...
async def _fetch_live_snapshot() -> SystemLiveUpdate:
    """
    Open a fresh DB session, build a live snapshot, and close the session.

    Redis-ready: replace the DB call with:
        value = await redis_client.get("system-health:live")
        return SystemLiveUpdate.model_validate_json(value)

    Kafka-ready: replace with an AIOKafka consumer reading from:
        topic = "system-health-events"
...
async def system_health_live_ws(websocket: WebSocket) -> None:
    """
    WebSocket endpoint handler for WS /ws/system-health/live.
    ...
    app.add_api_websocket_route("/ws/system-health/live", system_health_live_ws)

    Or with APIRouter (FastAPI >= 0.100):
    router.add_api_websocket_route("/live", system_health_live_ws)
    """
    ...
    try:
        ...
        logger.exception(
            "Failed to send initial WS payload to %s: %s", client_id, exc
        )
    ...
    try:
        ...
        # Currently the System Health feed is read-only so we just wait for disconnect.
        while True:
            data = await websocket.receive_text()
            # Future: parse client commands (e.g. filter by specific service)
